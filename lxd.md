...
```
Export a conainer to lxc image 
```
lxc publish local:yourcontainername --alias=yournewname
```
Export a container image to tar.gz
```
...
```
lxc image import /your/location/yourname --alias=newname
```
...
Here is a config that dont use bridge setup with netplan so if you dont want to make network change on the host.
...
config:
  ...
  user.user-data: |
    ...
    users:
      - name: user
        ssh-authorized-keys:
          - ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQCw0246LpRtBBJV4t9gsPRaGy5ZMfJAZ63fHN7EdBzZvWO6jrS5BdRGujt50sykXo54mpkbrKCngWqz+q5glqb$
          - ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQDznoT7SGEe/Yld35/FmQF0UWDkn/k4zZLggisEbnjr8xgWW0pNMvwt7gH9GvCQtTb5jDj2VYlTah52/00E73m$
          - ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAACAQDKowVHY6Y5Ar+AYAUeCJiMHH+UQNUZjN8De5dc/GKZOROIKYRxHAaPIgtvoHNNSU7w30p3gCvJaeFrqXr/HXF$
          - ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAACAQCtrbOooF4bEBdymEB9N5jh2mDCLAmWO5TCh0gorNOWyC5htBAJ0gqHGdh6OHqg2wT5sLDSMQ/FrpaOIdW5ULi$
        sudo: ['ALL=(ALL) NOPASSWD:ALL']
        groups: sudo
        shell: /bin/bash
description: Default LXD profile
